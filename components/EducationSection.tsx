"use client";
import { data } from "@/content/data";

export default function EducationSection() {
  return (
    <section id="education" className="container-prose py-16">
      <header className="mb-8">
        <h2 className="text-2xl font-semibold text-ink-900">Education</h2>
        <div className="mt-4 h-px bg-gradient-to-r from-transparent via-pink-200 to-transparent" />
      </header>

      <ul className="relative pl-6 md:pl-8">
        <span className="pointer-events-none absolute left-2 top-0 bottom-0 w-px bg-pink-200 md:left-3" />

        {data.education.map((e) => (
          <li key={`${e.school}-${e.period}`} className="relative mb-10 last:mb-0">
            <article className="card p-6 transition hover:-translate-y-1 hover:shadow-lg">
              <header className="flex items-start justify-between gap-6">
                <h3 className="font-semibold text-slate-900">
                  {e.school}
                </h3>
                <span className="text-sm text-slate-500 shrink-0">{e.period}</span>
              </header>

              <p className="mt-1 text-slate-700">{e.degree}</p>

              {e.details && (
                <p className="mt-3 text-sm rounded-md bg-pink-50 px-3 py-2 text-pink-700 border border-pink-200">
                  {e.details}
                </p>
              )}

             
              {e.involvement?.length ? (
                <ul className="mt-3 list-disc list-inside text-sm text-slate-600 space-y-1">
                  {e.involvement.map((x) => (
                    <li key={x}>{x}</li>
                  ))}
                </ul>
              ) : null}
            </article>
          </li>
        ))}
      </ul>
    </section>
  );
}
